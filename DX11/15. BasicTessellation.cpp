...
BasicTessellation::BasicTessellation(HINSTANCE hInstance)
	: App(hInstance)
{
	_mainWindowCaption = L"Basic Tessellation Demo";
	_enable4xMsaa = false;
...
void BasicTessellation::DrawScene()
{
	...
	Effects::TessellationFX->SetEyePosW(_eyePosW);
	...
	for (uint32 p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::TessellationFX->TessTech->GetPassByIndex(p)->Apply(0, _deviceContext.Get());
...
void BasicTessellation::OnMouseDown(WPARAM btnState, int32 x, int32 y)
{
	_lastMousePos.x = x;
	_lastMousePos.y = y;
...
void BasicTessellation::OnMouseUp(WPARAM btnState, int32 x, int32 y)
{
	ReleaseCapture();
}

void BasicTessellation::OnMouseMove(WPARAM btnState, int32 x, int32 y)
{
