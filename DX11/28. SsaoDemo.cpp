...
SsaoDemo::SsaoDemo(HINSTANCE hInstance)
	: App(hInstance)
{
	_mainWindowCaption = L"SSAO Demo";
	...
	_dirLights[0].Ambient = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	_dirLights[0].Diffuse = XMFLOAT4(0.5f, 0.5f, 0.4f, 1.0f);
...
void SsaoDemo::DrawScene()
{
	...
	_ssao->SetNormalDepthRenderTarget(_depthStencilView);
...
void SsaoDemo::OnMouseDown(WPARAM btnState, int32 x, int32 y)
{
	_lastMousePos.x = x;
	_lastMousePos.y = y;
...
void SsaoDemo::OnMouseUp(WPARAM btnState, int32 x, int32 y)
{
	ReleaseCapture();
}

void SsaoDemo::OnMouseMove(WPARAM btnState, int32 x, int32 y)
{
...
void SsaoDemo::DrawScreenQuad(ComPtr<ID3D11ShaderResourceView> srv)
{
