...
void SkinnedMeshRenderer::Render()
{
	...
	{
		if (m_MeshSubsetIndex >= m_SkinnedMesh->Subsets.size())
			break;
		...
		if (m_pMaterial == nullptr)
		{
			...
			Effects::InstancedBasicFX->SetMaterial(m_SkinnedMesh->Mat[m_MeshSubsetIndex]);
		...
		m_SkinnedMesh->ModelMesh.Draw(deviceContext, m_MeshSubsetIndex);
	}
}

void SkinnedMeshRenderer::RenderInstancing(shared_ptr<class InstancingBuffer>& buffer)
{
	if (m_SkinnedMesh == nullptr)
		return;
	...
	for (uint32 p = 0; p < techDesc.Passes; ++p)
	{
		...
		m_SkinnedMesh->ModelMesh.InstancingDraw(deviceContext, m_MeshSubsetIndex, buffer->GetCount());
	}
}
...
void SkinnedMeshRenderer::RenderShadowInstancing(shared_ptr<class InstancingBuffer>& buffer)
{
	if (m_SkinnedMesh == nullptr)
		return;
...
void SkinnedMeshRenderer::RenderShadowNormalInstancing(shared_ptr<class InstancingBuffer>& buffer)
{
	if (m_SkinnedMesh == nullptr)
		return;
...
{
	ImGui::Text("%s", m_SkinnedMesh ? "Mesh" : "None");
	...
	if (ImGui::Button("Select##Mesh"))
	{
		...
		if (!filePath.empty())
		{
			m_SkinnedMesh = ResourceManager::GetI()->LoadSkinnedMesh(filePath);
			if (m_SkinnedMesh)
			{
				m_MeshPath = filePath;
	...
	{
		// 서브셋 선택 드롭다운 구현
		if (ImGui::BeginCombo("Subset", m_SkinnedMesh->Subsets[m_MeshSubsetIndex].Name.c_str())) // The second parameter is the previewed value
		{
			for (int n = 0; n < m_SkinnedMesh->Subsets.size(); n++)
			{
				bool is_selected = (m_MeshSubsetIndex == n); // You can store your selection somewhere
				if (ImGui::Selectable(m_SkinnedMesh->Subsets[n].Name.c_str(), is_selected))
					m_MeshSubsetIndex = n;
				if (is_selected)
